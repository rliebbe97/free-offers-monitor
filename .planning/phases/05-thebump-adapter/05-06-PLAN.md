...
<objective>
Create unit tests for scraping-utils.ts (ScrapeError, extractExternalId, fetchWithRetry, respectfulDelay), base-forum-adapter.ts (pagination loop, stop conditions, challenge detection, shouldSkipPost, partial results on error), and thebump-adapter.ts (extractPostsFromPage, getNextPageUrl, date parsing, sticky/admin skip) using HTML fixtures and mocked fetch.
</objective>
...
### Task 2: Create base-forum-adapter.test.ts

<type>execute</type>
<files>apps/worker/src/ingestion/base-forum-adapter.test.ts</files>
...
<action>
Create `apps/worker/src/ingestion/base-forum-adapter.test.ts` to test the BaseForumAdapter abstract class via a minimal concrete subclass.
...
<acceptance_criteria>
- File exists at `apps/worker/src/ingestion/base-forum-adapter.test.ts`
- Contains a concrete `TestForumAdapter` subclass for testing abstract methods
- Tests single-page extraction, max-pages stop, since-date stop, challenge detection, shouldSkipPost, and partial results on error
- All mocks follow the same pattern as other test files (vi.mock hoisting)
- Uses `vi.clearAllMocks()` in `beforeEach`
</acceptance_criteria>

<verify>
grep "describe('BaseForumAdapter'" apps/worker/src/ingestion/base-forum-adapter.test.ts && grep 'TestForumAdapter' apps/worker/src/ingestion/base-forum-adapter.test.ts && grep 'thebump_pagination_stop' apps/worker/src/ingestion/base-forum-adapter.test.ts && grep 'thebump_challenge_detected' apps/worker/src/ingestion/base-forum-adapter.test.ts && grep 'forum_page_fetch_failed' apps/worker/src/ingestion/base-forum-adapter.test.ts
</verify>

### Task 3: Create thebump-adapter.test.ts
...
3. `grep -c 'describe(' apps/worker/src/ingestion/base-forum-adapter.test.ts` returns at least 1
4. `grep -c 'describe(' apps/worker/src/ingestion/thebump-adapter.test.ts` returns at least 6 (one per concern)
5. No test imports any actual network or DB dependency
</verification>
...
- All base-forum-adapter tests pass: single-page extraction, max-pages stop, since-date stop, challenge detection, shouldSkipPost short body filter, partial results on fetch error
